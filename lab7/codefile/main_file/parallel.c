...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel private(randx, randy, i) 
	{
		int id = omp_get_thread_num();
		int seeds = ((id & 0xFFFFFFF0) | (id + 1));

		#pragma omp for reduction(+:cir_pnts) private(seeds)
		for (i = 0; i < interval; i++)
		{
			randx = rand_r(&seeds)/(double)RAND_MAX;
            randy = rand_r(&seeds)/(double)RAND_MAX;
